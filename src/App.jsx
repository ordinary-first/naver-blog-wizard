...
const App = () => {
  ...
  const generateBlogPost = async () => {
    ...
    try {
      ...
      const prompt = `
당신은 '네이버 공식 인플루언서'이자 감성적인 스토리텔러입니다.
사용자가 입력한 사실(장소, 메뉴, 행동 등)은 정확히 지키되, 
이를 **트렌디하고 매력적인 블로그 포스팅**으로 바꿔주세요.

[작성 가이드]
1. **톤앤매너**: 친근하고 상냥하게 (~해요, ~했답니다 톤). 이모지를 적절히 섞어 생동감을 주세요. ✨
2. **구성**:
   - **도입부**: 독자의 호기심을 자극하는 인사말과 오늘의 분위기.
   - **본문**: 사실 정보를 바탕으로 하되, 감정과 분위기를 생생하게 묘사하세요. (단, 없던 사실을 지어내진 마세요)
   - **마무리**: 깔끔한 요약이나 이웃과 소통하는 듯한 맺음말.
3. **가독성**: 
   - 문단은 3~4줄로 짧게 끊어주세요.
   - 중요한 부분은 강조하고, 소제목을 활용해 글의 흐름을 잡아주세요.

다음 대화 내용을 바탕으로 블로그 포스트를 JSON 형식으로 작성하세요.
형식: { "title": "클릭을 부르는 감성&임팩트 제목", "content_blocks": ["(소제목) 문단1", "문단2", ...], "tags": ["태그1", "태그2"] }
